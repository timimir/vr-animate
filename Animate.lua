...
function CustomAnimate.Init(Character, Animator)
    ...
    local function IsRunning(currentSpeed)
        ...
        return currentSpeed >= (walkSpeed + 1)
    end
    ...
    -- Возврат к стандартной анимации (Idle / Walk / Run)
    local function ReturnToStandardAnim()
        CurrentPriority = 0
    ...
    _G.PlayCustomAnim = function(animSource)
        -- Получаем данные анимации для проверки Loop
        local animData
        if type(animSource) == "string" then
            animData = Anims[animSource] or Animator:GetAnim(animSource)
        else
            animData = animSource
        end
        ...
        -- Если анимация зациклена (Loop=true) — играем бесконечно, пока не Stop
        if animData and animData.Loop == true then
            return
        end

        -- Автосброс по окончании для незацикленных анимаций
        if realDuration and realDuration > 0 then
            task.delay(realDuration + 0.1, function()
                if CurrentPriority <= 5 then 
                    ReturnToStandardAnim()
                end
            end)
        end
    end

    _G.StopCustomAnim = function()
        ReturnToStandardAnim()
    end

    PlayAnim("Idle", "Core")
    print("CustomAnimate Initialized.")
end
